...
module DemoSeeder
  ...
  def self.users
    demo_user_details = {
      username: "finaldemo",
      password: "password",
      email: "final@demo.gov",
      avatar_url: "www.goodby.com/wave.png"
    }
    user = User.new(demo_user_details)
    if user.save
      @user_id = user.id
    else
      raise "we have a serious problem!!!"
    end
  end
  ...
  def self.errands
    ...
    lat: "42.084431",
    lng: "-87.929582",
    task: "find that phone I lost on mt. prospect",
    npc_id: (1..n).to_a.sample,
    quest_id: (1..q).to_a.sample,
    hero_id: @user_id
    ...
    lat: "42.075098",
    lng: "-87.690096",
    task: "gaze at the lake and think about aging",
    npc_id:  (1..n).to_a.sample,
    quest_id: (1..q).to_a.sample,
    hero_id: @user_id
    ...
    lat: "41.847946",
    lng: "-87-766466",
    task: "pick up custom posterboard",
    npc_id: (1..n).to_a.sample,
    quest_id: (1..q).to_a.sample,
    hero_id: @user_id
    ...
    lat: "41.796963",
    lng: "-87.611382",
    task: "rake leaves in washington park",
    npc_id: (1..n).to_a.sample,
    quest_id: (1..q).to_a.sample,
    hero_id: @user_id
  ...
  def self.equips
    equip_info = {
      owner_id: @user_id,
      lootable_id: Item.find_by(rarity: 4001).id,
      lootable_type: 'Item'
    }
    Equip.create(equip_info)
  end
